...
class MainWin(tk.Tk):
    ...
    def __init__(self):
        ...
        self.file_name = tk.StringVar() # 文件名称
    ...
    def show_base_info(self):
        ...
        row3 = tk.Frame(self)
        row3.pack(fill='x', padx=1, pady=5)
        tk.Label(row3, text="模板文件名称：", width=15).pack(side=tk.LEFT)
        tk.Entry(row3, textvariable=self.file_name, width=self.label_width, justify=tk.CENTER).pack(side=tk.LEFT)
        ...
        tk.Label(row4, text="收件人邮箱名单：", width=15).pack(side=tk.LEFT)
    ...
    def set_default_info(self):
        '''设置默认初始值'''
        self.subject.set('Subject:{}年{}月xxxx'.format(*self._get_year_month()))
        self.file_name.set('default.html')
        try:
...
class SendEmail(object):
    ...
    def _make_mail_text(self, sender, sender_name, sign, date, info_row):
        curpath = os.getcwd()
        path = os.path.join(curpath, self.win.file_name.get())
        mail_text = open(path, 'r').read()
